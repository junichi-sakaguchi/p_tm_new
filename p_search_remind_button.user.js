...
(() => {
  ...
  function ensureButtons() {
    ...
    // 再注入抑止
    if (bar.querySelector('.tm-btn-remind') && bar.querySelector('.tm-btn-gohonen')) return;
  ...
  async function onGohonen() {
    ...
    selectDiv.click();
    const option = await waitForOptionByText(LABELS.ngReasonText, 3000);
    if (!option) return alert(`「${LABELS.ngReasonText}」の候補が見つかりません`);
    ...
    memo.focus();
    setReactInputValue(memo, LABELS.gohonenMemo);
